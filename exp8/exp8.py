# -*- coding: utf-8 -*-
"""
Created on Fri Nov 12 11:14:56 2021
...
def match(filename1, filename2, method):
    ...
    if (method == 'surf'):
        ...
        bf = cv2.BFMatcher(cv2.NORM_L2, crossCheck=True)  # 新建BFMatcher对象，surf的normType应该使用NORM_L2或者NORM_L1
        matches = bf.match(des1, des2)
        matches = sorted(matches, key=lambda x: x.distance)
        knnMatches = bf.knnMatch(des1, des2, k=1)  # drawMatchesKnn
    if(method == 'orb'):
        ...
        bf = cv2.BFMatcher(cv2.NORM_HAMMING, crossCheck = True) # 新建BFMatcher对象，orb的normType应该使用NORM_HAMMING
        matches = bf.match(des1, des2)
        matches = sorted(matches, key=lambda x: x.distance)
        knnMatches = bf.knnMatch(des1, des2, k = 1) # drawMatchesKnn
    # 过滤
...
def main():
    ...
    for i in range(3):
        match('./pic/huawei.jpg', './pic/2.jpg', method[i])
